									...
									switch ($Column['Input']) {
											...
											echo '<select name="'.$Column['Field'].'" '.($Column['Required']=='true'?'required':null).'>';
												if (!empty($Column['SetEnum'])) {
													...
													foreach ($data as &$val) {
														$tmp = explode(':',$val);
														echo '<option '
															.($Data[$Column['Field']] == $tmp[0]?'selected':null)
															.' value="'.$tmp[0].'">'.$tmp[1];
													}
												}
											echo '</select>';
										...
										case 'checkboxes':
												...
												foreach ($data as &$val) {
													...
													echo '<input 
														type="checkbox" '
														.($Data[$Column['Field']] == $tmp[0]?'checked':null)
														.'
														name="'.$Column['Field'].'[]" 
														value="'.$tmp[0].'">'.$tmp[1];
												}
										...
										case 'checkbox':
												...
												echo '<input  
													type="checkbox" '
													.($Data[$Column['Field']] == $data[0]?'checked':null)
													.'name="'.$Column['Field'].'" 
													value="'.$data[0].'">'.$data[1];
											break;
										case 'number':
											echo '<input 
												'.($Column['Required']=='true'?'required':null).' 
												type="number" '
										...
										case 'text':
											echo 
											"<input ".($Column['Required']=='true'?'required':null)." type='text' name='".$Column['Field']."' 
												".(!empty($Column['Max'])
													?"maxlength='$Column[Max]'"
													:null)." "
												.(!empty($Column['Min'])
										...
										case 'file':
											...
											break;
										default:
											echo "<input ".($Column['Required']?'required':null)."
												name='$Column[Field]' 
												type='$Column[Input]' 
												value='".$Data[$Column['Field']]."' 
												/>";
