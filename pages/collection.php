...
$Collections = mysqli_query($db,$query);
*/

if (isset($_GET['page'])) {
	$onpage = 12;
	$pages = mysqli_fetch_assoc(mysqli_query($db,"SELECT COUNT(*) FROM `Dresses_$lang`"));
	$pages = round($pages['COUNT(*)']/$onpage+0.5);
	if ($_GET['page'] < 1)
		$page = 1;
	else
		$page = $_GET['page'];
...
$query = "SELECT * FROM `Dresses_$lang` LIMIT $limit,12";
$Collection = mysqli_query($db,$query);
...
<head>
	...
	<title>Каталог свадебных платьев </title>
	<meta name="description" content="На сайте присутствует каталог свадебных платьев, где Вы сможете просмотреть шикарные свадебные платья, эксклюзивные свадебные платья." />
    <meta name="keywords" content="Свадебные платья от производителя, Свадебные платья Черновцы, Купить свадебные платья, Каталог свадебных платьев, Шикарные свадебные платья, Эксклюзивные свадебные платья, Платье Perla, Свадебное платье на заказ, Модель Camelia, кордовое кружево" />
							...
							<div class="main-menu">
								...
								<nav>
									...
									<ul>
										...
										<a href="collection.php" class="active"><li><?=$Interface['Collection']?></li></a>
		...
		<div class="container">
			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					<div class="content-block content-block-coll">
						
						<?php
							
							while ($Dress = mysqli_fetch_assoc($Collection)) {

						?>
						
						<!-- main-block -->
						<a href="wedding-dress.php?Id=<?=$Dress['Id']?>">
							<?php
								if (is_file('../img/collections/'.$Dress['Id'].'/1.jpg')) {
									$img = '../img/collections/'.$Dress['Id'].'/1.jpg';
								} elseif(is_file('../img/collections/'.$Dress['Id'].'/2.jpg')) {
									$img = '../img/collections/'.$Dress['Id'].'/2.jpg';
								} elseif(is_file('../img/collections/'.$Dress['Id'].'/3.jpg')) {
									$img = '../img/collections/'.$Dress['Id'].'/3.jpg';
								} elseif(is_file('../img/collections/'.$Dress['Id'].'/4.jpg')) {
									$img = '../img/collections/'.$Dress['Id'].'/4.jpg';
								}
							?>
							<div class="article-block" style="background: url('../img/collections/<?=$Dress['Id']?>.jpg') no-repeat;background-size:cover;">

								<div class="article-block-text">

									<article><?=$Dress['Name']?></article>

								</div><!-- end article-block-text-->
